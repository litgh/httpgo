...
func (s *Scanner) scanRawJSON() rune {
	ch := s.Peek()
	ch = s.skipWhitespace(ch)
	var count int
	for {
		switch ch {
		case '[', '{':
...
redo:
	...
	switch {
	case s.tok == Header || s.tok == File || s.tok == Field:
		s.scanString(' ')
		s.tok = String
		ch = s.next()
	case s.tok == Param:
	...
	if s.tok != Param && s.tok != Header && s.tok != Field && s.tok != File && s.tok != RawJSON {
		// end of token text
